...
<i>Send /stop to Stop Process</i>""")
        ...
        async with session_lock:
            if session_dict['CONFIRM_PHN'].lower() in ['y', 'yes']:
                break
    try:
        pyro_client = Client(f"Cloud-Station-{message.from_user.id}", api_id=api_id, api_hash=api_hash)
...
<i>Send /stop to Stop Process</i>""")
 ...
 <i>Send /stop to Stop Process</i>""")
    ...
    try:
        ...
        await pyro_client.send_message("self", f"⌬ <b><u>Pyrogram Session Generated :</u></b>\n\n<code>{session_string}</code>\n\n<b>Via <a href='https://telegram.me/cloud_station9'>Cloud Station 🔥</a> [ @cloud_station9 ]</b>", disable_web_page_preview=True)
        await pyro_client.disconnect()
    ...
    try:
        await aioremove(f'Cloud-Station-{message.from_user.id}.session')
        await aioremove(f'Cloud-Station-{message.from_user.id}.session-journal')
    except Exception:
        pass
